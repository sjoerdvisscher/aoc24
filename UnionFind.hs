...
updateAnn :: UnionFind s m -> Node -> (m -> m) -> ST s ()
...
union :: UnionFind s m -> Node -> Node -> (m -> m -> m) -> ST s ()
union uf@(UnionFind {..}) x y f = do
  x <- find uf x
  y <- find uf y
  when (x /= y) $ do
    sx <- readArray sz x
    ...
    if sx < sy
      then do
        ...
        writeArray ann y (f mx my)
      else do
        writeArray parent y x
        writeArray sz x (sx + sy)
        writeArray ann x (f mx my)
